...
public class ConstraintsResolver {
    ...
    public void typeResolver(int type) {
        switch (type) {
            ...
            case 3:
                ...
                break;
            case 4:
                constrains.add(from_object + ".x == " + parent + ".x");
                constrains.add(from_object + ".y == " + parent + ".y");
    ...
    public String resolveType7(int attr1, int constant) {
        switch (attr1) {
            ...
            case 5:
                left = true;
                if (attr1 > 0) return from_object + ".left==0dp" + ((constant >= 0) ? "-" + constant : "+" + Math.abs(constant)) + "dp";
                break;
            case 6:
                right = true;
                if (attr1 > 0)
